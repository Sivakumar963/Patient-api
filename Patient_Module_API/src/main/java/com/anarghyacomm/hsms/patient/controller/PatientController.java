...
public class PatientController {
	...
	@GetMapping("/patient/{email}")
	   public Optional<PatientEntity> getByUserMail(@PathVariable  String email) {
			
		return service.getByEmail(email);
		   
	   }
	@GetMapping("/patients/{patientID}")
     public Optional<PatientEntity> getPatientId(@PathVariable int patientID) {
			
	  return service.getPatientId(patientID);
